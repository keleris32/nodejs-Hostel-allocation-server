...
const getAvailableRooms = async (req, res) => {
    try {
        ...
        const availableRooms = await dbConnector_1.default.query('SELECT id, hostel, room_type, room_number, price, no_of_inhabitants, type_of_hostel FROM rooms WHERE type_of_hostel = $1 AND no_of_inhabitants < $2', [studentGender, roomCapacity]);
        if (availableRooms.rowCount !== 0) {
            let responseMessage = 'Hostel rooms fetched successfully!';
            res
                .status(200)
                .json((0, successResponse_1.successResponseBody)(responseMessage, (0, randomizeRooms_1.randomizeRooms)(availableRooms.rows)));
        }
        else {

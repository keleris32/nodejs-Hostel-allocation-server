...
const registerStudent = async (req, res) => {
    ...
    try {
        const student = await dbConnector_1.default.query('SELECT matric_no FROM students WHERE matric_no = $1', [matric_no]);
        if (student.rowCount !== 0) {
            throw new Error('This matric number has been used!');
        }
        const hashedPassword = await (0, bcryptjs_1.hash)(password, 12);
        await dbConnector_1.default.query('INSERT INTO students (name, level, course, matric_no, password, gender) VALUES ($1, $2, $3, $4, $5, $6)', [name, level, course, matric_no, hashedPassword, gender]);
        ...
        if (error.message === 'This matric number has been used!') {
            ...
            res.status(422).json((0, errorResponse_1.errorResponseBody)('Invalid credentials provided!'));
        }
        else {
